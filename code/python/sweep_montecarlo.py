# -*- coding: utf-8 -*-
"""
SWEEP — Monte Carlo, sensibilidad y estabilidad.

Se ejecuta SOBRE LA CONFIGURACIÓN CONGELADA post-WFO (results/wfo_results_sweep.json).
"""
...
def collect_trades(params: SweepParams, assets: list, upto=HOLDOUT_START) -> list:
    pooled = []
    for s in assets:
        df = load_asset(s)
        if df is None:
        ...
        if len(sub) < 3000:
            continue
        eng = SweepEngine(sub, params, symbol=s, equity0=EQUITY0)
        eng.run()
...
def equity_dd(r: np.ndarray) -> float:
    """Equity COMPUESTO (cumprod) desde el diseño -- ver auditoria de
    hydra_montecarlo.py: la version aditiva (cumsum) puede producir equity
    negativo y drawdowns >100%, matematicamente imposibles."""
...
def mc_frictions(trades: list, iters: int, rng, base: SweepParams) -> dict:
...
NON_OPT_PARAMS = ["structure_lookback_h1", "vol_ma_window", "ema_trail_n",
...
def sensitivity(base_overrides: dict, assets: list) -> list:
    ...
    for pname in NON_OPT_PARAMS:
        base_val = getattr(SweepParams(), pname)
...
def stability(overrides: dict, assets: list) -> dict:
    ...
    for s in assets:
        ...
        if len(sub) < 3000:
            continue
        eng = SweepEngine(sub, p, symbol=s, equity0=EQUITY0)
        eng.run()
...
def get_config() -> dict:
    fn = "results/wfo_results_sweep.json"
...
def main():
    ...
    with open("results/montecarlo_results_sweep.json", "w", encoding="utf-8") as f:
        json.dump(out, f, indent=2, ensure_ascii=False)

    print("\n=== RESUMEN MONTE CARLO SWEEP ===")
    ...
    print("\n[OK] resultados -> results/montecarlo_results_sweep.json")
