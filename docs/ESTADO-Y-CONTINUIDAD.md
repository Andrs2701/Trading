...
Se han probado **6 hipótesis de trading algorítmico** (4 tendenciales en cripto + 1 en forex/materias primas + 1 de reversión a la media), todas con el mismo protocolo de rigor (formalización → diagnóstico → multi-activo → Walk-Forward Optimization → Monte Carlo → veredicto, con holdout intocable y prohibición explícita de ajustar parámetros a mano mirando resultados). **Las 6 fueron rechazadas formalmente. La Hipótesis 6 (pairs trading) aportó el hallazgo más valioso del proyecto — mean-reversion resuelve el problema de drawdown catastrófico — pero su edge concreto no se sostuvo al aislarlo.**
...
| 6 | **Pairs Trading**: reversión a la media del spread entre criptos cointegradas | ❌ NO APROBADO (edge concentrado en 1 par, no sobrevive aislado) | `docs/PAIRS-formalizacion.md`, `docs/PAIRS-resultados-veredicto.md` |

Las hipótesis 1, 2, 3 y 5 carecen de edge in-sample/out-of-sample creíble. La Hipótesis 4 (BREAKOUT-ATR) arrojó una expectativa neta combinada positiva pero con riesgo de cola severo (Monte Carlo: drawdown de hasta -95% en escenarios plausibles, sin importar cuántos activos correlacionados se agreguen — ver §11). La Hipótesis 6 (pairs trading) demostró que mean-reversion **sí resuelve** ese problema de drawdown (-21%/-36% contra -78%/-96% en cualquier variante tendencial) — pero al investigar de dónde salía el resultado positivo del pool H4, se encontró que dependía al 100% de un solo par (ETHUSDT-XRPUSDT); sin él el resultado colapsa a cero, y aislado tampoco pasa el chequeo de sobreajuste (muestra insuficiente). Riesgo resuelto, edge concreto todavía sin confirmar — ver §12.
...
## 12. Hipótesis 6 — Pairs Trading / Reversión a la Media (❌ NO APROBADO, pero el hallazgo de riesgo más valioso del proyecto)
...
- **Monte Carlo (H4, pool completo)**: drawdown bootstrap **-20.8% p95 / -36.1% peor caso** — dramáticamente mejor que cualquier variante tendencial (-78% a -96%). Sensibilidad: 0 de 8 parámetros críticos, primera vez en el proyecto. Fricciones estresadas: pasa.
- **Investigando la alarma de concentración por par (139.4%)**: se corrieron los dos cortes que responden si el resultado es real o depende de un solo par. **Sin ETHUSDT-XRPUSDT** (5 pares): el resultado colapsa a mean_oos≈0, WFE=-0.01 — NO RENTABLE OOS. **Solo ETHUSDT-XRPUSDT aislado**: mean_oos se ve aún mejor (+0.546) pero WFE=0.28 — SOBREOPTIMIZACIÓN (muestra OOS chica, 124 trades, IS sospechosamente alto).
- **Veredicto final**: NO APROBADO en los tres cortes probados (pool completo, sin el par dominante, o el par dominante solo). El hallazgo de que mean-reversion resuelve el riesgo de cola es real y queda validado — ningún corte de pares mostró nada cerca de los drawdowns catastróficos de las hipótesis tendenciales — pero el edge concreto de esta implementación específica no se sostiene. Camino de continuación: ampliar el universo de pares candidatos más allá de los 11 activos ya usados (no forzar más análisis sobre los 6 pares ya descartados).
